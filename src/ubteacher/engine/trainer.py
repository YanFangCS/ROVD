...
class BaselineTrainer(DefaultTrainer):
    ...
    def update_img_memory(self, cfg, valid,img_memory):
        for data in valid:
            if 'memory' not in data['image_id']:
                ...
                for obj in objs:
                    ...
                    elif len(img_memory[class_id]) < cfg.ICARL.NUM_PC:
                        img_memory[class_id].append(ele)          
                    else:
                        del_id = random.randint(0,cfg.ICARL.NUM_PC)
                        if del_id != cfg.ICARL.NUM_PC:
                            img_memory[class_id][del_id] = ele
        return img_memory

    def get_memory(self, img_memory):
        dataset_dicts = []
